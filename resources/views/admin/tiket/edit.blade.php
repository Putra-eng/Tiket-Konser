    ...
    <div class="bg-white rounded-lg shadow-md p-8">
        <form action="{{ route('admin.tiket.update', $tiket->id_tiket) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="number" id="harga" name="harga" value="{{ $tiket->harga }}" class="w-full px-4 py-2 border-2 rounded-lg focus:outline-none" style="border-color: var(--color-gold)" required min="0" step="1000">
                @error('harga')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" id="stok" name="stok" value="{{ $tiket->stok }}" class="w-full px-4 py-2 border-2 rounded-lg focus:outline-none" style="border-color: var(--color-gold)" required min="0">
                @error('stok')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
